...
When the user asks Codex to use Abraxius for a script change:
...
Do not make Codex calculate diffs, construct `multi_edit` payloads, escape Luau
source into PowerShell, create scripts manually, or perform a second read-back
for a normal successful push. `push` owns source discovery, targeted edit
generation, new-script creation, MCP `multi_edit`, and pending tracking.
