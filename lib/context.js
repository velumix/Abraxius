...
class MCPContext {
  ...
  ingestStudioEvent(event) {
    ...
    if (type === "context_snapshot" && event.snapshot) {
      // A snapshot can race a SelectionChanged event and arrive with an empty
      // Selection:Get() result. Preserve the most recent non-empty selection
      // event so AI context does not regress to "nothing selected".
      const previousSelection = this.studioContext?.selectionPaths;
      this.studioContext = event.snapshot;
      if (Array.isArray(previousSelection) && previousSelection.length > 0 &&
          (!Array.isArray(this.studioContext.selection) || this.studioContext.selection.length === 0)) {
        this.studioContext.selectionPaths = previousSelection;
      }
