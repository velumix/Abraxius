...
function buildAiContext(options = {}) {
  ...
  return {
    ...
    abraxiusNotice: ABRAXIUS_NOTICE,
...
function toMarkdown(aiContext) {
  ...
  lines.push("## Abraxius MCP Notice");
  lines.push("- Presence: Abraxius Roblox Studio MCP Bridge is present in this repository.");
  lines.push("- Host: http://localhost:13470 (Abraxius App supervised host)");
  lines.push("- Prerequisites: App host running; Roblox Studio open with Abraxius companion plugin.");
  lines.push("- Recommended MCP command: `node cli.js stdio` (or `node cli.js push <file>` for script changes).");
  lines.push("- Safety Rule: Roblox Studio script mutations require explicit user intent and approval. Inspect live state before mutating.");
  lines.push("");
  ...
  if (aiContext.github) {
    const { toGitHubMarkdown } = require("./github-context");
    lines.push(toGitHubMarkdown(aiContext.github).trimEnd());
    lines.push("");
  }
