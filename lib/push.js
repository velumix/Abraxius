...
class Pusher {
  ...
  async push(localFile) {
    ...
    if (!health.connected) {
      return this._pushWithCompanion(studioPath, newSource, properties);
    }
  ...
  async _pushWithCompanion(studioPath, newSource, properties) {
    const current = await this.client.pluginCall({
      type: "read_source",
      path: studioPath,
    });
    const currentResult = current && current.result ? current.result : current;
    if (!currentResult || currentResult.ok !== true) {
      throw new Error(
        `Companion cannot update ${studioPath}: ${currentResult?.error || "script not found"}. ` +
        "Creating new scripts still requires MCP.",
      );
    }

    const update = await this.client.pluginCall({
      type: "set_properties",
      path: studioPath,
      properties: { Source: newSource, ...(properties || {}) },
    });
    const updateResult = update && update.result ? update.result : update;
    if (!updateResult || updateResult.ok !== true) {
      throw new Error(
        `Companion failed to update ${studioPath}: ${updateResult?.error || "unknown error"}`,
      );
    }

    const verification = await this.client.pluginCall({
      type: "read_source",
      path: studioPath,
    });
    const verificationResult = verification && verification.result
      ? verification.result
      : verification;
    if (!verificationResult || verificationResult.source !== newSource) {
      throw new Error(`Companion source verification failed for ${studioPath}`);
    }

    return {
      changed: currentResult.source !== newSource,
      studioPath,
      result: { transport: "companion", verified: true },
    };
  }
}
