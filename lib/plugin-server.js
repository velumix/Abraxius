...
const EXPORT_COMMAND_TIMEOUT_MS = 120000;
...
class PluginSession {
  ...
  enqueue(command) {
    ...
    return new Promise((resolve, reject) => {
      const timeoutMs = command?.type === "export_scripts"
        ? EXPORT_COMMAND_TIMEOUT_MS
        : COMMAND_TIMEOUT_MS;
  ...
  async start() {
        ...
        switch (route) {
          ...
          case "POST /plugin/register": {
            ...
            if (session) {
              ...
              session = new PluginSession(sessionId, metadata);
    ...
    return new Promise((resolve, reject) => {
      this.server.listen(this.port, () => {
        ...
        logger.success(`Plugin companion server listening on http://localhost:${this.port}`);
        resolve();
      });
